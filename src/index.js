...
/*
    Documentation:
        ...
        3) add props.withFadeClassName variable to component className prop. withFade 
        exposes additional classes created based on consumer input from storyblok plugin
        4) add style props "style={props.withFadeStyle}"
outl
        ...
        2. 
            fade_plugin: {
                type: "custom",
                field_type: "component-fade-plugin",
                options: [],
                description: "Plugin for fading components into view."
            },
        3. <div className={`text-block ${props.withFadeClassName}`} style={props.withFadeStyle}>
...
export const withFade = WrappedComponent => options => {
    ...
    if (window && window.navigator.userAgent.indexOf("Trident") !== -1) {
      return props => {
        return (
          <WrappedComponent
            {...props}
            withFadeClassName="before-fade after-fade"
          />
        );
      };
    }
  ...
  return props => {
    // early return when fade not enabled
